...
The installer creates a local Python virtual environment, installs `codex-sync-kit`, installs the Codex plugin source, and prints the next command for adding the plugin to Codex.
...
codex-sync restore --snapshot latest
```
...
```text
Use $codex-sync to scan my Codex setup and push a safe snapshot.
```
